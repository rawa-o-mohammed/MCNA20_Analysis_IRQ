msni_recoding <- function(df, loop) {
  ...
  df$e3 <- case_when(fsc$hhs[match(df$X_uuid, fsc$X_uuid)] >= 2 ~ 1,
                     fsc$hhs[match(df$X_uuid, fsc$X_uuid)] < 2 ~ 0,
                     TRUE ~ NA_real_)
  ...
  df$lsg_health_vulnerable <-
    case_when(df$lsg_health == 1 &
                df$vulnerability_score >= 3 ~ 1, TRUE ~ 0)
  ###################################h #########################################
